...
import static com.assissoft.canif.simcalc.utils.FuncoesHelper.calculaFinanciamento_4;

/**
 * Created by Marcos on 10/10/2016.
 */
public class Financiamento_4Fragment extends Fragment {
    ...
    private final TextWatcher textWatcher = new TextWatcher() {
        ...
        public void onTextChanged(CharSequence s, int start, int before, int count) {
            //after text changed
            calculaFinanciamento_4(periodo.getText().toString(), valor_financiado.getText().toString(), taxa_de_juros.getText().toString(), valor_da_parcela, valorTotalPagar, valorGastoComJuros);
        }
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        valor_financiado = view.findViewById(R.id.et_valor_financiado);
        ...
        prazoPagto = view.findViewById(R.id.tv_prazo_pgto_primeira_parcela);
        primeiraParcemEm = view.findViewById(R.id.tv_data_primeira_parcela_em);
        valorTotalPagar = view.findViewById(R.id.tv_valor_total_a_pagar_informado);
        taxaAnualJuros = view.findViewById(R.id.tv_tx_anual_juros_informado);
        valorGastoComJuros = view.findViewById(R.id.tv_valor_gasto_de_juros_informado);

        valor_financiado.addTextChangedListener(textWatcher);
        taxa_de_juros.addTextChangedListener(textWatcher);
        periodo.addTextChangedListener(textWatcher);

        //Campos não editáveis
        valor_da_parcela.setEnabled(false);
    ...
    private String getTaxaAnualJuros() {
        ...
        if (taxa_de_juros.getText().toString().length() != 0) {
            ...
            if (Locale.getDefault().getLanguage().equals("en"))
                resultado = ConverteUnidadeHelper.calculaEquivTaxas("month-year", taxa_de_juros.getText().toString());
            else
                resultado = ConverteUnidadeHelper.calculaEquivTaxas("mês-ano", taxa_de_juros.getText().toString());
